...
- **Current scope**: a `PLATFORM_ADMIN` today has platform powers (list/edit any company) **and** acts as a normal `ADMIN` on their *own* company only — doesn't see other companies' data. Full cross-company access is wanted eventually but deliberately deferred (2026-09-01); two undecided approaches: make `company_id` nullable plus a `?company=X` targeting param, or an "impersonation" mode.
