...
// Matches shared/models/document-line.ts's DocumentLine now that the DB has
// a real document_sections table (see document_section.repository.ts) -
// both types share quantity+unit for their amount (e.g. "20 Sac" for a
// material, "5 Heure" for a service), no separate time field. No `reference`
// either - document_lines has no such column.
...
// Sections/lines are still built up as pure local state while editing (same
// as before), but "Valider" now actually creates everything server-side:
// POST /document, then POST /document-section per section, then POST
// /document-line per line under it (sequentially - each line needs its real
// section_id, which only exists once that section's own create resolves).
// On success, navigates to the created document's detail page, same as
// document-form.ts (v1).
@Component({
  selector: 'app-document-form-v2',
  standalone: true,
  imports: [FormsModule, DecimalPipe, InputText, InputNumber, Textarea, FloatLabel, Select, SelectButton, DatePicker, Button, Card],
...
export class DocumentFormV2 implements OnInit {
  ...
  // filter of its own. Same approach as document-form.ts (v1).
  ...
  ngOnInit(): void {
    ...
    // Load the default type's template too - onTypeChange only fires on an
    // actual change from here on, and nothing has changed yet at init.
  ...
  async submit(): Promise<void> {
    ...
    try {
      const document = await firstValueFrom(this.documentService.createDocument({
        ...
        // afterwards, not at creation, same as document-form.ts (v1).
        status: 'DRAFT',
      }));
      ...
      // Same landing spot as v1 - the server-generated number is shown on
      // the document's own detail page.
